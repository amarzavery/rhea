...
import { ICompositeType, types } from "./types";
...
frames.read_frame = function (buffer) {
  ...
  if (frame.type === FrameType.TYPE_AMQP) {
...
frames.amqp_frame = function (channel, performative, payload) {
  return { 'channel': channel || 0, 'type': FrameType.TYPE_AMQP, 'performative': performative, 'payload': payload };
};
frames.sasl_frame = function (performative) {
  return { 'channel': 0, 'type': FrameType.TYPE_SASL, 'performative': performative };
};

function define_frame(type, def: TypesModule.ICompositeType) {
...
define_frame(FrameType.TYPE_AMQP, open);
define_frame(FrameType.TYPE_AMQP, begin);
define_frame(FrameType.TYPE_AMQP, attach);
define_frame(FrameType.TYPE_AMQP, flow);
define_frame(FrameType.TYPE_AMQP, transfer);
define_frame(FrameType.TYPE_AMQP, disposition);
define_frame(FrameType.TYPE_AMQP, detach);
define_frame(FrameType.TYPE_AMQP, end);
define_frame(FrameType.TYPE_AMQP, close);

var sasl_mechanisms = {
  name: 'sasl_mechanisms', code: 0x40,
  fields: [
...
define_frame(FrameType.TYPE_SASL, sasl_mechanisms);
define_frame(FrameType.TYPE_SASL, sasl_init);
define_frame(FrameType.TYPE_SASL, sasl_challenge);
define_frame(FrameType.TYPE_SASL, sasl_response);
define_frame(FrameType.TYPE_SASL, sasl_outcome);
