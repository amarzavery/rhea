...
function define_section(descriptor: any, unwrap: any, wrap?: any): void {
...
function define_composite_section(def) {
  ...
  let wrap = function (sections, msg) {
...
function define_map_section(def) {
  let descriptor: any = { numeric: def.code };
...
message.decode = function (buffer) {
  let msg = new Message();
  let reader = new types.Reader(buffer);
  while (reader.remaining()) {
    let s = reader.read();
    log.message('decoding section: %o of type: %o', s, s.descriptor);
    if (s.descriptor) {
      let unwrap = unwrappers[s.descriptor.value];
      if (unwrap) {
        unwrap(msg, s);
      } else {
...
function define_outcome(def) {
  ...
  message['is_' + def.name] = function (o) {
    if (o && o.descriptor) {
      let c = outcomes[o.descriptor.value];
      if (c) {
...
message.unwrap_outcome = function (outcome) {
  if (outcome && outcome.descriptor) {
    let c = outcomes[outcome.descriptor.value];
    if (c) {
